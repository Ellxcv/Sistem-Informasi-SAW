...
<div class="container mt-5">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <a class="btn btn-success" href="{{ route('kendaraan.create') }}"> Create New Kendaraan</a>
    </div>
    ...
    <table class="table table-bordered">
        <tr>
            ...
            <th>
                Kapasitas
                <button type="button" class="btn btn-icon btn-outline-secondary" onclick="sortData('kapasitas')">
        ...
        <tr>
            ...
            <td>
                <form action="{{ route('kendaraan.destroy', $kendaraan->id_kendaraan) }}" method="POST">
                    <a class="btn btn-primary" href="{{ route('kendaraan.edit', $kendaraan->id_kendaraan) }}">Edit</a>
                    @csrf
                    @method('DELETE')
    ...
    <div class="d-flex justify-content-center"> 
        ...
        <nav aria-label="Page navigation">
            <ul class="pagination">
                <!-- Tombol Previous -->
                <li class="page-item {{ $kendaraans->currentPage() == 1 ? 'disabled' : '' }}">
                ...
                <li class="page-item {{ $kendaraans->currentPage() == $i ? 'active' : '' }}">
                    <a class="page-link" href="{{ $kendaraans->url($i) }}">{{ $i }}</a>
                </li>
                @endfor
                <!-- Tombol Next -->
                <li class="page-item {{ $kendaraans->currentPage() == $kendaraans->lastPage() ? 'disabled' : '' }}">
...
<script>
    ...
    function sortData(column) {
        ...
        window.location.href = `{{ route('kendaraan.index') }}?orderBy=${column}&orderDirection=${sortDirection}`;
    }

    function changePageSize(size) {
        window.location.href = "{{ route('kendaraan.index') }}?pageSize=" + size;
    }
</script>
